...
def _is_garbage_identifier(val: str) -> bool:
    """Check if the SMBIOS identifier is a VM/OEM placeholder or garbage string."""
    normalized = val.strip().lower()
    if not normalized:
        return True
    garbage_keywords = {
        'ffffffff', '0000000000', 'none', 'default string', 
        'to be filled by o.e.m.', 'to be filled by oem', 
        'not specified', 'unknown', 'system serial number', 
        '1234567890', 'fill in at oem', 'to be filled by o.e.m',
        'not applicable', 'n/a', 'invalid', 'null',
        '00000000-0000-0000-0000-000000000000'
    }
    if normalized in garbage_keywords:
        return True
    if len(normalized) > 1 and all(c == normalized[0] for c in normalized):
        return True
    return False
...
class Vault:
    ...
    def _get_machine_id(self, reject_garbage: bool = True) -> bytes:
        """
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                for part in result.stdout.strip().split('|'):
                    part_str = part.strip()
                    if part_str:
                        if reject_garbage and _is_garbage_identifier(part_str):
                            continue
                        elif not reject_garbage and part_str == 'FFFFFFFF':
                            continue
                        identifiers.append(part_str.encode('utf-8'))
        except Exception:
            pass
    ...
    def _stream_decrypt_file(self, src: Path, dst: Path, file_key: bytes) -> None:
        """
        AES-256-CTR decrypt src to dst. HMAC verified before rename.
        Writes to dst.with_suffix('.tmp') first, renames on success.
        Raises VaultError on HMAC mismatch or header corruption.
        """
        with open(src, 'rb') as inp:
        ...
        # HMAC verified — atomically rename tmp to final destination
    ...
    def _load_raw(self) -> Dict[str, Any]:
        ...
        if not _hmac.compare_digest(computed, stored_hmac):
            # Try with unfiltered machine seed for backward compatibility
            unfiltered_seed = self._get_machine_id(reject_garbage=False)
            unfiltered_integrity_key = HKDF(
                algorithm=hashes.SHA256(),
                length=32,
                salt=b"GodsHands-vault-integrity-v1",
                info=b"vault-integrity",
            ).derive(unfiltered_seed)
            
            clean = {"schema": data.get("schema", self.SCHEMA), "items": data.get("items", {})}
            canonical = json.dumps(clean, sort_keys=True, separators=(',', ':'))
            computed_unfiltered = _hmac.new(unfiltered_integrity_key, canonical.encode('utf-8'), 'sha256').hexdigest()
            
            if _hmac.compare_digest(computed_unfiltered, stored_hmac):
                self._machine_seed = unfiltered_seed
                self._vault_integrity_key = unfiltered_integrity_key
            else:
